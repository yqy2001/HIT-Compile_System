...
句柄是句型的最左直接短语（高度为2的子树的边缘？）问题就在于如何正确地识别句柄。
...
<img src="https://i.loli.net/2021/05/01/8JxTAE5oOgYD6rn.png" alt="image-20210501101545753" style="zoom: 40%;" /><img src="https://i.loli.net/2021/05/01/CltwLAiWO1U5JQk.png" alt="image-20210501101649021" style="zoom:40%;" />
...
若文法G对应的DFA的每个状态都满足这个条件：1、没有移进-归约冲突；2、没有归约-归约冲突，则**此文法是LR(0)文法**。
...
移入规约冲突的根本原因：无法确定当前可归约串是否为句柄。需要更多的信息来判断是否是句柄，是否能规约。
...
项目集中的归约项目的终结符的FOLLOW集两两不相交，移进符号不属于任何一个FOLLOW集。也就是SLR分析表没有冲突。
...
2、CLOSURE函数：$[A→\alpha B.\beta,a]$扩展出$[B→.\gamma,b]$，$b\in FIRST(\beta a)$。若$\beta$可以推出空，b=a叫做继承的后继符，否则叫自生的。
